...
func (h *HandlerRoutes) GetStudentsHandler(w http.ResponseWriter, r *http.Request) {
	...
	filter := structs.IPaginationFilter{
		Filter: name,
		Fields: []string{"last_name", "first_name"},
	}
	res, err := paginator.GetPaginatedData(h.DB, page, limit, &students, &filter)
	...
	response := structs.IGetStudentsResponse{
		Data:     students,
