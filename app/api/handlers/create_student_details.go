...
func (h *HandlerRoutes) CreateStudentDetailsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := tx.Create(&studentInput).Error; err != nil {
		...
		zapLog.Error("error create student detail",
			zap.Error(err),
		)
	...
	if !isTransactionSuccess {
		http.Error(w, "Error create student detail.", http.StatusInternalServerError)
		return
	}
